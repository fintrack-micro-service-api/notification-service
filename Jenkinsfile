pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'fintrack-notification-service'
        CONTAINER_NAME = 'fintrack-notification-service-container'
    ...
    stages {
        ...
        //                     -Dsonar.projectKey=fintrack-notification-service \
        //                     -Dsonar.sources=src \
        ...
        stage('Trigger ManifestUpdate') {
            steps {
                script {
                    try {
                        build job: 'notification-service-pipeline-2', parameters: [string(name: 'DOCKERTAG', value: env.BUILD_NUMBER)]
                    } catch (Exception e) {
                        currentBuild.result = 'FAILURE'
